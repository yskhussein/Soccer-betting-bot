...
def api_football(endpoint: str, params: dict) -> dict:
    """Call API-Football v3. Supports both direct API key and RapidAPI key."""
    try:
        # Try direct api-sports.io first
        ...
        data = r.json()
        # Check if auth failed — then try RapidAPI format
        ...
        if errors:
            logger.warning(f"API-Football errors ({endpoint}): {errors}")
        remaining = r.headers.get("x-ratelimit-requests-remaining", "?")
        if remaining != "?":
            logger.info(f"API-Football: {endpoint} — {remaining} requests remaining today")
        return data
    except Exception as e:
...
async def run_daily_tips():
    ...
    if not fixtures:
        ...
        account = test.get("response", {})
        requests_used = account.get("requests", {}).get("current", "?")
        requests_limit = account.get("requests", {}).get("limit_day", "?")
        await notify(
