...
# ── CONFIG ────────────────────────────────────────────────────────────────────
# API Budget: 1500 req/day
# Per match analysis uses ~5 calls (fixture, stats x2, h2h, odds)
# So we can analyze ~200 matches/day comfortably
...
def get_current_season() -> int:
    """Return current football season year. Seasons run Aug-May so 2025 = 2024/25."""
    now = datetime.now(timezone.utc)
    return now.year if now.month >= 7 else now.year - 1

def api_football(endpoint: str, params: dict) -> dict:
    """Call API-Football v3. Auto-detects correct auth format."""
    # Try 3 different auth methods in order
    attempts = [
        # Method 1: Direct api-sports.io key header
        {
            "url":     f"https://v3.football.api-sports.io/{endpoint}",
            "headers": {"x-apisports-key": API_FOOTBALL_KEY},
        },
        # Method 2: RapidAPI format
        {
            "url":     f"https://v3.football.api-sports.io/{endpoint}",
            "headers": {
        ...
        # Method 3: Alternative api-football.com base URL
        {
            "url":     f"https://api-football-v1.p.rapidapi.com/{endpoint}",
            "headers": {
                "x-rapidapi-key":  API_FOOTBALL_KEY,
    ...
    for i, attempt in enumerate(attempts):
        try:
            ...
            # If token error, try next method
            if errors and ("token" in str(errors).lower() or "key" in str(errors).lower()):
                logger.debug(f"Auth method {i+1} failed for {endpoint}: {errors}")
                continue
            # Success
            if i > 0:
                logger.info(f"Auth method {i+1} worked for API-Football!")
            remaining = r.headers.get("x-ratelimit-requests-remaining", "?")
            if remaining != "?" and endpoint == "fixtures":
                logger.info(f"API-Football: {remaining} requests remaining today")
            return data
        except Exception as e:
            logger.debug(f"Auth method {i+1} exception for {endpoint}: {e}")
            continue
    logger.error(f"All API-Football auth methods failed for {endpoint}")
    return {"errors": {"token": "All auth methods failed"}, "response": []}


def get_todays_fixtures() -> list:
    """Get all fixtures for today. First tries per-league, falls back to all-fixtures date query."""
    today   = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    season  = get_current_season()
    all_fixtures = []

    # Method 1: Fetch all fixtures for today in one call (most efficient)
    data = api_football("fixtures", {"date": today})
    bulk = data.get("response", [])
    logger.info(f"Bulk fixtures for {today}: {len(bulk)} found (season auto-detected)")
    ...
    if bulk:
        for f in bulk:
            try:
                ...
                all_fixtures.append({
                    ...
                    "league":     f"{league_name} ({country})" if country else league_name,
                    "league_id":  league_id,
                    "home":       home,
                    "away":       away,
                    "kickoff":    kickoff,
        ...
        logger.info(f"Total upcoming fixtures today: {len(all_fixtures)}")
        return all_fixtures

    # Method 2: Fallback — try per tracked league with season
    logger.info("Bulk fetch returned 0 — trying per-league fallback")
    for league_name, league_id in LEAGUE_IDS.items():
        data = api_football("fixtures", {
            "league": league_id, "date": today, "season": season
        })
        ...
        for f in fixtures:
            try:
                ...
                logger.warning(f"Fixture parse error {league_name}: {e}")
        if fixtures:
...
def get_team_stats(team_id: int, league_id: int) -> dict:
    ...
    data = api_football("teams/statistics", {
        "team": team_id, "league": league_id, "season": get_current_season()
    })
...
def get_injuries(team_id: int) -> list:
    ...
    data = api_football("injuries", {"team": team_id, "season": get_current_season()})
    injured = []
...
def format_daily_summary(tips: list) -> str:
    ...
    for i, tip in enumerate(tips[:25], 1):
...
async def run_daily_tips():
    ...
    if not fixtures:
        ...
        await notify(
            ...
            f"Date checked: {datetime.now(timezone.utc).strftime('%Y-%m-%d')}\n"
            f"API requests used: {requests_used}/{requests_limit}\n"
            f"Season: {get_current_season()}\n\n"
            f"Possible causes:\n"
            f"• API key exhausted (100/day free limit)\n"
            f"• No matches scheduled today\n"
            f"• Check API key in Railway variables"
        )
        return
    ...
    for fixture in fixtures[:100]:  # 1500 req/day allows generous coverage
        try:
            # Get team IDs
            ...
            await asyncio.sleep(0.5)  # 1500 req/day allows faster scanning
...
async def scheduler(app_tg):
    ...
    # Validate API-Football key on startup
    test   = api_football("status", {})
    errors = test.get("errors", {})
    if errors:
        logger.error(f"API-Football key error: {errors}")
        await notify(
            f"⚠️ <b>API-Football Key Error!</b>\n"
            f"{list(errors.values())[0] if errors else 'Unknown error'}\n\n"
            f"➡️ Fix: Railway Variables → check API_FOOTBALL_KEY\n"
            f"Get key: https://dashboard.api-football.com"
        )
    else:
        resp = test.get("response", {})
        if isinstance(resp, list): resp = resp[0] if resp else {}
        used  = resp.get("requests", {}).get("current", "?")
        limit = resp.get("requests", {}).get("limit_day", "?")
        logger.info(f"API-Football ✅ — {used}/{limit} requests used today")
...
@flask_app.route("/api/test_key")
def api_test_key():
    """Test the API key and return diagnostic info."""
    import requests as _req
    key = API_FOOTBALL_KEY
    results = {}

    # Test 1: Direct api-sports header
    try:
        ...
        results["direct_apisports"] = {"error": str(e)}

    # Test 2: RapidAPI header
    try:
        r2 = _req.get("https://v3.football.api-sports.io/status",
                      headers={"x-rapidapi-key": key,
                               "x-rapidapi-host": "v3.football.api-sports.io"}, timeout=10)
        data2 = r2.json()
        ...
        results["rapidapi"] = {
            ...
            "errors": errors2,
            "account": resp2.get("account", {}),
            "requests": resp2.get("requests", {}),
        }
    except Exception as e:
        results["rapidapi"] = {"error": str(e)}

    results["key_preview"] = key[:8] + "..." if key else "NOT SET"
    results["key_length"]  = len(key) if key else 0
